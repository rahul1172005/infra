  ...
  return (
    ...
    >
      ...
      <nav className="hidden xl:flex items-center gap-6">
        ...
        <Link href="/dashboard" className="hover:opacity-70 transition-opacity">OPERATION CENTER</Link>
        ...
        {isAuthenticated && user ? (
          <div className="flex items-center gap-4">
            <div className="hidden sm:flex flex-col items-end leading-none">
              <span className="text-[10px] font-black tracking-tighter mb-0.5">{user.nickname || user.name || 'ZAPSTER'}</span>
            </div>
            {user.picture ? (
              <img src={user.picture} alt="Profile" className="w-8 h-8 rounded-full border border-black/10" />
            ) : (
              <div className="w-8 h-8 rounded-full bg-black flex items-center justify-center text-[#FFD700] text-[10px]">
                {user.email.charAt(0).toUpperCase()}
              </div>
            )}
            <button
              onClick={() => logout()}
              className="text-[9px] hover:text-red-700 transition-colors border-l border-black/10 pl-4 h-8 flex items-center"
            >
              LOGOUT
            </button>
          </div>
        ) : (
          <Link href="/auth/login" className="bg-black text-[#FFD700] px-6 py-2 border border-black hover:bg-black transition-colors uppercase tracking-[0.2em] text-[10px] font-bold">
            BOOST SECURITY
          </Link>
        )}
      </div>
