...
export function ActionCard({
    ...
    return (
        <div className={`relative min-h-[500px] md:min-h-[600px] flex flex-col items-center justify-center p-8 md:p-12 lg:p-20 overflow-hidden text-center group transition-all rounded-[2rem] md:rounded-[3.5rem] ${
            isOversight 
            ...
            <div className="relative z-10 space-y-8 md:space-y-12 max-w-4xl px-4 w-full">
                {icon && (
                    <div className="flex items-center justify-center mx-auto">
                        {typeof icon === 'boolean' ? (
                            <GOTIcon size={64} scale={1.2} x={0} y={0} className={isOversight ? "text-white" : "text-black"} />
                        ) : (
                            icon
                        )}
                    </div>
                ...
                <div className="space-y-6 md:space-y-8">
                    <div className="space-y-2">
                        {subtitle && (
                            <p className={`text-[10px] tracking-[0.6em] font-black uppercase mb-2 ${
                                isOversight ? 'text-white/20' : 'text-black/20'
                            }`}>
                                {subtitle}
                            </p>
                        )}
                        <h2 className={`text-5xl md:text-7xl lg:text-8xl font-black uppercase tracking-[0.1em] leading-relaxed border-b-4 pb-4 md:pb-6 inline-block ${
                            isOversight ? 'text-white border-[#E81414]' : 'text-black border-black'
                        }`}>
                            {title}
                        </h2>
                    </div>
                    
                    <div className={`p-8 md:p-12 border max-w-2xl mx-auto backdrop-blur-sm rounded-[2rem] md:rounded-[2.5rem] ${
                        isOversight ? 'bg-black border-white/5' : 'bg-[#F9F9F9] border-black'
                    }`}>
                        <p className={`text-[14px] md:text-[16px] tracking-wide font-black uppercase leading-relaxed ${
                            isOversight ? 'text-white/50 group-hover:text-white' : 'text-black'
